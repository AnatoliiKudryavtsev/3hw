...
while True:
    inventory_tuple_list.append(
        (input('Введите номер товара: '), dict({
            ...
            'Цена': float(input('Укажите цену: ')),
            'Количество': int(input('Введите количество товара: ')),
            'Ед': str(input('Выберите единицы измерения (например, шт): ')),
        }))
    )

    if input('\nТовар добавлен. Добавить еще один товар? (да/нет): ') == 'нет':
        break

    i += 1

# Вывод списка товаров:
print(f'\nСписок товаров:{inventory_tuple_list}')
...
# Вывод аналитики по товарам:
print(f'Аналитика: {output_dict}')
